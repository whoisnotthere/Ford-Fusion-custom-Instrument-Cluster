...
Item {
    ...
    function update_Trip_Data() {
        let distance = TRIPS.return_SinceLastRestart_Distance.toFixed(1);
        trip_Distance_Value.text = String(distance).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1,');

        let time = TRIPS.return_SinceLastRestart_Time;
        trip_Time_Value.text = seconds_To_String(time);
        trip_Time_Unit.text = update_Time_Name(time);

        trip_Consumption_Value.text = TRIPS.return_SinceLastRestart_Efficiency.toFixed(1);
    }
    ...
    Connections {
        target: TRIPS
        function onSinceLastRestart_Data_Changed() {
            update_Trip_Data();
        }
